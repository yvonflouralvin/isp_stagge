...
export default function StageMasterPageCS(props: PageProps) {
    ...
    const load = async (page: number, search?: string) => {
        const url = `/isp_stage/stage-master/?page=${page}${search !== undefined ? `&search=${search}` : ""}`
        try {
    ...
    const { } = useEvent((eventId, { }) => {
        if (eventId === "list-stage-master-creation") load(total_pages)
    }, ["list-stage-master-creation"]);
            ...
            <div className="mt-[10px]">
                <div>
                    ...
                    <div>
                        ...
                        {
                            results.map((item: StageMaster, index) => {
                                return <Link href={`/apps/isp_stage/stage-masters/${item.id}`} key={item.id} className="duration-300 flex gap-[2px] text-[13px] text-gray-500 px-[20px] py-[6px] my-[3px] cursor-pointer hover:bg-[rgba(0,0,0,0.02)] border-b border-inherent">
