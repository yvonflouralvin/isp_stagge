                ...
                <ModalBody>
                    <div>
                        ...
                        <div className='mt-[10px]'>
                            <p className='font-light text-gray-500 text-[13px]'>Nombre d'étudiants par professeur</p>
                            {isUpdating === false && <p className='font-bold'>{department_settings ? `${department_settings.max_teacher_memoire}` : ""}</p>}
                            {isUpdating === true && <input className='border-0 outline-none w-full bg-transparent text-[13px]' ref={nbetudiants} id="nbgroups" defaultValue={department_settings ? `${department_settings.max_teacher_memoire}` : ""} placeholder="Nombre d'étudiants par professeur" type='number' />}
                        </div>
