...
export default function AddUsers(props: Props) {

    const [isOpen, setIsOpen] = React.useState<boolean>(false);
    const [isSaving, setIsSaving] = React.useState(false);
    const [error, setError] = React.useState<string | undefined>();
    const [selectedSection, setSelectedSection] = React.useState<string | undefined>();
    const [selectedDepartement, setSelectedDepartement] = React.useState<string | undefined>();
    ...
    const handleSubmit = (e: any) => {
        ...
        api(cookies).post(`/isp_stage/dept-recherche-officier/`, {
            name: data.name,
                        ...
                        <div className="flex flex-col gap-[5px]">
                            ...
                            <Select
                                ...
                                {props.sections.map((section: any, index: number) => (
                                    <SelectItem key={section.id} value={section.id} textValue={`${section.libelle}`}>
                                        {section.libelle}
                                    </SelectItem>
                                ))}
                            </Select>
                            ...
                            {
                                selectedSection !== undefined && <Select
                                    required={true}
                                    ...
                                    label="Departement"
                                    className="w-full" >
                                    {props.gradeclasses.filter(dept =>dept.grade.id === selectedSection).map((dept: any, index: number) => (
                                        <SelectItem key={dept.id} value={dept.id} textValue={`${dept.libelle}`}>
                                            {dept.libelle}
                                        </SelectItem>
                                    ))}
                                </Select>
